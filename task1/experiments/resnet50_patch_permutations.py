...
class STL10SelectedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        original_index = self.selected_indices[idx]
...
def patch_shuffle(image, permutation):
    ...
    patches = patches.reshape(GRID_SIZE * GRID_SIZE, 3, PATCH_SIZE, PATCH_SIZE)
    ...
    shuffled = shuffled.permute(2, 0, 3, 1, 4)
    shuffled = shuffled.reshape(3, IMAGE_SIZE, IMAGE_SIZE)
...
def create_permutations(num_images):
    ...
    for _ in range(num_images):
        while True:
            permutation = rng.permutation(GRID_SIZE * GRID_SIZE)
...
permutations = create_permutations(len(dataset))
...
with torch.no_grad():
    ...
    for batch_idx, (images, labels_batch) in enumerate(loader):
        ...
        batch_permutations = permutations[batch_start:batch_end]
        ...
        clean_features = resnet(images_normalized)
        shuffled_features = resnet(shuffled_normalized)

        clean_logits = classifier(clean_features)
        shuffled_logits = classifier(shuffled_features)
...
shuffled_accuracy = correct_shuffled / total
consistency = same_predictions / total
...
print(f"Accuracy drop: {accuracy_drop * 100:.2f} percentage points")
print(f"Prediction consistency: {consistency * 100:.2f}%")